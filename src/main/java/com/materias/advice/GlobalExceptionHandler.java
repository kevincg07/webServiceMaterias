...
import com.materias.errors.ApIErrors;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler{
	...
	protected ResponseEntity<Object> handleMissingPathVariable(MissingPathVariableException ex, HttpHeaders headers,
		...
		String message = ex.getMessage();
		List<String> details = new ArrayList<>();
		details.add("Paath variable es missing");
		ApIErrors errors = new ApIErrors(message, details, status, LocalDateTime.now());
		return ResponseEntity.status(status).body(errors);
	}
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
		...
		String message = ex.getMessage();
		List<String> details = new ArrayList<>();
		details.add("Request body in not readable");
		ApIErrors errors = new ApIErrors(message, details, status, LocalDateTime.now());
		return ResponseEntity.status(status).body(errors);
	}

	@ExceptionHandler(IdNotDeleteException.class)
	public ResponseEntity<Object> handleIdNotDeleteException(IdNotDeleteException ex){
		String message = ex.getMessage();
		List<String> details = new ArrayList<>();
		details.add("Id does not exist");
